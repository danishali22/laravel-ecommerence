...
class CartController extends Controller
{
    ...
    public function processCheckout(Request $request)
    {
        ...
        $check = PartialPayment::latest()->first();
        ...
        if ($request->payment_method == "stripe") {
            ...
            try {
                ...
                // $check = PartialPayment::where('order_id', $order->id)->get;
                // dd($check);
                
                
                $partial = new PartialPayment();
                $partial->user_id = Auth::user()->id;
                $partial->order_id = $order->id;
                $partial->stripe_token = $request->stripeToken;
                $partial->name_on_card = $request->name_on_card;
                $partial->card_number = $request->card_number;
                $partial->cvc = $request->cvc;
                $partial->exp_month = $request->exp_month;
                $partial->exp_year = $request->exp_year;
                $partial->total_receipts = $numRecipients;
                $partial->total_receipts_paid_for = $amountPaidFor;
                $partial->remaing_receipts = $numRecipients - $amountPaidFor;
                $partial->total_amount = $totalAmount;
                $partial->total_paid_amount = $paidAmount;
                $partial->paid_amount = $paidAmount;
                $partial->remaining_amount = $totalAmount - $paidAmount;
                $partial->user_desc = $user_desc;
                $partial->save();
